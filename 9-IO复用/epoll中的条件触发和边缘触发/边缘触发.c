...
#define BUF_SIZE 4      // 这里特意将缓冲区减小，就是为了阻止服务器端一次性读取接收的数据
...
int main(int argc, char* argv[])
{
    ...
    while (1)
    {
        ...
        // 对于每一个客户端而言，由于缓冲区太小根本不够存，因此无可避免要在while循环
        // 作用下去反复读取，而每次读取就意味着epoll_wait函数的又一次注册，即满足了
        // 条件触发的特点，只要缓冲区有数据，就要提交注册
        ...
        for(int i = 0; i < event_cnt; i++)
        {
            if (ep_events[i].data.fd == serv_sock)  // 建立连接前的握手需要读取所传递的信息
            {
                ...
                setnonblockingmode(clnt_sock);      // 设置为非阻塞方式
                
                // 边缘触发的改动，而这里的改动会导致客户端出现错误
                // 由于边缘触发只注册一次，就要求读取输入缓冲中的全部数据
                // 而如果到时候缓冲区为空，就会陷入阻塞，因此设置为非阻塞方式
            ...
            else    // 已经建立好了连接，这部分则是正常的信息收发
            {
                while(1)    // while循环不断读取，直到缓冲区读取完毕
                {
                    ...
                    {
                        if(errno == EAGAIN) break;  // read函数返回-1且errno值为EAGAIN时，说明读取完毕
                    }
                    else
                    {
